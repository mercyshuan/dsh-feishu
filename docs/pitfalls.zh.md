...
- **被丢弃的群消息对 agent 是不可见的——要靠 surface 自己的缓冲找回。**
  在 `always` 模式下，每条未被 @ 的群消息都会在 `shouldRespond` 之后被丢弃，
  这意味着：用户连发三条、只在最后一条 @，agent 就只拿到最后一条；而当这条
  @ 本身没有正文时，这一轮是**空的**（只有身份块）。由此有两个坑。
  （1）入站缓冲必须在门禁**之前**记录，而不是之后：被门禁丢弃的消息恰恰是
  之后那次空 @ 需要吸收的内容；而 allowlist 已使记录它们变得安全——合并
  绝不跨越发送者，因此被允许用户的回合不会吸收被拒用户的话。
  （2）回溯必须在**换了发送者**和**投递水位线**（已交给 agent 的最新消息）
  处停止——跨着别人的发言去捞同一用户最近 N 条，会把从未发生的对话拼接在
  一起；而把会话里已有的内容再合并一次，则是让 agent 重复读一遍。
- **平台历史正文里仍带着 `@_user_<n>` 占位符。**
  `parseMessageBody` 清掉的是实时事件用的 `<at …>` 形式，而
  `im.v1.message.list` 把提及序列化进正文，形如 `@_user_1`。直接渲染进
  agent 上下文会泄漏原始占位符，因此历史正文在合并前要过一遍与实时消息
  相同的 `stripMentions`。
- **`im.v1.message.list` 的时间参数是秒。** `start_time` / `end_time` 是
  秒级时间戳字符串；surface 内部用毫秒，必须在 seam 处换算。传毫秒会静默地
  请求一个远在未来约 1000 倍的窗口（返回空页，看起来像"那段时间没人说话"）。
